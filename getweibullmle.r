Weibull.mle <- function(data)
{
  ...
  if(op$convergence == 0){
    ...
    sfit <- survreg(sobj~1, dist = "weibull", weights = weights)
    # tt <- tryCatch(survreg(sobj~1, dist = "weibull", weights = weight, control = survreg.control(maxiter=1000)), warning = function(w) w)
